...
public class BootBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (PlayLogicUtil.isInLauncherGuide()) {
            LogUtil.e(TAG, "it is in guiding,so not open AD");
            return;
        }
        ...
        if (Intent.ACTION_BOOT_COMPLETED.equals(action)) {
            ...
            if (Utils.isNetConnected(context)) {
                ...
                AdInfo mFullScreenAds = PlayLogicUtil.getFullScreenAds();
                if (mFullScreenAds != null && !MyApplication.getInstance().isOpened()) {
                    MyApplication.getInstance().setOpened(true);
                    PlayLogicUtil.bootStartActivity(context, mFullScreenAds);
                }
            }
            context.sendBroadcast(new Intent(BroadcastAction.NET_CHANGE));
