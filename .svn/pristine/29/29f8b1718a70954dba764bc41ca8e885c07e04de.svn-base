...
public class PlayLogicUtil {
        ...
        if (mAdResources.size() > 0) {
            ...
            if (mFullScreenAds.getType() == TypeParam.TYPE_SINGLE_PHOTO || mFullScreenAds.getType() == TypeParam.TYPE_MULTIPLE_PHOTO) {
                if (InterfaceService.hasCachImage(context, mFullScreenAds)) {
                    ...
                    MyApplication.getInstance().setOpened(false);
                    return;
                }
    ...
    public static void playDefault(Context context) {
        ...
        defoultIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        try {
    ...
    public static void goHome(Context context) {
        ...
        if (mFullScreenAds != null) {
            ...
            if (mFullScreenAds.getType() == TypeParam.TYPE_SINGLE_PHOTO || mFullScreenAds.getType() == TypeParam.TYPE_MULTIPLE_PHOTO) {
                ...
                PlayLogicUtil.startActivity(context, PhotoActivity.class, mFullScreenAds, IntentValue.AD_POSITION_BOOT);
            } else {
                if (InterfaceService.hasCachVideo(context, mFullScreenAds)) {
                    PlayLogicUtil.playDefault(context);
                } else {
                    LogUtil.i(TAG, "SplashActivity video");
                    PlayLogicUtil.startActivity(context, VideoActivity.class, mFullScreenAds, IntentValue.AD_POSITION_BOOT);
                }
