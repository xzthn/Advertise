...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        LogUtil.e(TAG, new ProxyCacheException(error).getMessage());
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            LogUtil.i("TAG" + Thread.currentThread().getName(), "Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            LogUtil.i("TAG" + Thread.currentThread().getName(), "Error reading ping response " + e.getMessage());
            return false;
        } finally {
            source.close();
        }
